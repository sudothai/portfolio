...
<section class="mt-12">
    <h1 class="text-6xl font-bold mb-8"><span class="underline text-6xl font-bold">Let</span>'s talk</h1>
    <div class="text-2xl grid xl:grid-cols-2 gap-8">
        ...
        {:else}
            <form action="" class="sm:order-2 md:order-2 lg:order-1" id="contact-form">
                <input type="text" class="block w-full mb-4 p-4 bg-[#F6F6F6] rounded-xl" class:input-error={isFormInvalid && !Boolean(contactName.length)} placeholder="Your Name" bind:value={contactName} />
                <input type="text" class="block w-full mb-4 p-4 bg-[#F6F6F6] rounded-xl" class:input-error={isFormInvalid && !Boolean(contactMail.length)} placeholder="Your Email" bind:value={contactMail} />
                <textarea name="" id="" placeholder="How can we help?" class="block w-full px-4 pt-4 pb-16 bg-[#F6F6F6] rounded-xl" class:input-error={isFormInvalid && !Boolean(informationAboutProject.length)} bind:value={informationAboutProject}></textarea>
                <Button onclick={onSubmit}>Submit</Button>
            </form>
        {/if}
        <div class="form-text sm:order-1 md:order-1 lg:order-2">
